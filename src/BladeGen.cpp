...
void BladeGenerator::initializeRange(float startXVar, float startYVar,
                                     float changeYVar, float changeXVar,
                                     float endXVar, float endYVar)
{
...
void BladeGenerator::generateSword(int seed)
{
    ...
    // drawer.gradientLine(0, 0, 0, 100, Vector4{100, 0, 0, 255},
    //     Vector4{0, 200, 0, 255});
    srand(seed);
    generateBlade(seed);
    generateHilt(seed);
...
void BladeGenerator::generateBlade(int seed)
{
    ...
    if (sym)
        {
            // determines a random start and end point using the given
            // parameters when initialized
            Vector2 startPoint;
            startPoint.x =
                rand() % int(mWidth * mStartXVar) + int(mWidth * .05);
            startPoint.y =
                rand() % int(mBladeHeight * mStartYVar) + mHiltHeight;
            Vector2 endPoint;
            endPoint.x = 0;
            endPoint.y = rand() % int(mBladeHeight * mEndYVar) + mHiltHeight;

            // determines the amount of times the line changes on its path to
            // the end point
            int32_t changes;
            changes = rand() % 15 + 3;

            // draws all the lines between each change point until the end point
            // is reached
            Vector2 newPoint;
            Vector2 oldPoint;
            oldPoint = startPoint;
            for (size_t i = 0; i < changes; i++)
                {
                    newPoint.x =
                        abs(oldPoint.x + rand() % int(mWidth * mChangeXVar) -
                            int(mWidth * (.5 * mChangeXVar)));
                    newPoint.y = oldPoint.y +
                                 rand() % int(mBladeHeight * mChangeYVar) +
                                 int(mBladeHeight * .05);
    ...
    else
        {
            ...
            Vector2 startPointL;
            startPointL.x =
                (rand() % int(mWidth * mStartXVar) + int(mWidth * .05)) * -1;
            startPointL.y =
                rand() % int(mBladeHeight * mStartYVar) + mHiltHeight;

            Vector2 endPoint;
            endPoint.x =
                rand() % int(mWidth * mEndXVar) - int(mWidth * mEndXVar * .5);
            endPoint.y = rand() % int(mBladeHeight * mEndYVar) + mHiltHeight;
            ...
            for (size_t i = 0; i < changesL; i++)
                {
                    newPoint.x = -1 * abs(oldPoint.x +
                                          rand() % int(mWidth * mChangeXVar) -
                                          int(mWidth * mChangeXVar * .5));
                    newPoint.y = oldPoint.y +
                                 rand() % int(mBladeHeight * mChangeYVar) +
                                 int(mBladeHeight * .05);
